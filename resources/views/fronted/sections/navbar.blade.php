...
<nav id="mainNav" class="navbar navbar-expand-lg align-items-center css3animate fnt-bold" role="navigation">
    <div class="px-3 px-md-5 d-flex justify-content-between align-items-center w-100">
        <h1 class="m-0">
            <button class="navbar-brand css3animate bg-transparent border-0 p-0 m-0" data-animation="22" data-goto="1">
                @if(!empty($general->image_logo_header_dark) || !empty($general->image_logo_header_light))
                    @if(!empty($general->image_logo_header_dark))
                        <img src="{{url('/')}}/{{$general->image_logo_header_dark}}" alt="{{$general->title}}"
                             class="img-fluid logo-header-dark"/>
                    @endif
                    @if(!empty($general->image_logo_header_light))
                        <img src="{{url('/')}}/{{$general->image_logo_header_light}}" alt="{{$general->title}}"
                             class="img-fluid logo-header-light"/>
                    @endif
                @else
                    {{$general->title}}
                @endif
        ...
        <ul class="nav navbar-nav d-none d-lg-flex {{$margin_css}}">
            ...
            @if($section->about_enable == true)
                <li class="nav-item">
                    <button class="p-0 pt-trigger css3animate" data-animation="22"
                            data-goto="@php echo $page_number++; @endphp" data-style="{{$section->about_scheme_color}}">
            ...
            @if ($section->projects_enable == 1)
                <li class="nav-item">
                    <button class="p-0 pt-trigger work-menu css3animate" data-animation="22"
                            data-goto="@php echo $page_number++; $page_number++; @endphp"
                            data-style="{{$section->projects_scheme_color}}">
            ...
            @if ($section->blog_enable == 1)
                <li class="nav-item">
                    <button class="p-0 pt-trigger blog-menu css3animate" data-animation="22"
                            data-goto="@php echo $page_number++; @endphp" data-style="{{$section->blog_scheme_color}}">
            ...
            @if ($section->contact_enable == 1)
                <li class="nav-item">
                    <button class="p-0 pt-trigger work-menu css3animate" data-animation="22"
                            data-goto="@php echo $page_number++; @endphp"
                            data-style="{{$section->contact_scheme_color}}">
...
<nav class="mobile-nav">
    <ul class="w-100 py-3 px-0">
        ...
        @if ($section->about_enable == 1)
            <li class="py-3 text-end">
                <button class="pt-trigger bg-transparent border-0" data-animation="22"
                        data-goto="@php echo $page_number_mobile++; @endphp"
                        data-style="{{$section->about_scheme_color}}">
                    {{$section->about_menu}}
                </button>
            </li>
        @endif
        ...
        @if ($section->projects_enable == 1)
            <li class="py-3 text-end">
                <button class="pt-trigger bg-transparent border-0" data-animation="22"
                        data-goto="@php echo $page_number_mobile++; $page_number_mobile++; @endphp"
                        data-style="{{$section->projects_scheme_color}}">
        ...
        @if ($section->blog_enable == 1)
            <li class="py-3 text-end">
                <button id="blogButton" class="pt-trigger bg-transparent border-0" data-animation="22"
                        data-goto="@php echo $page_number_mobile++; @endphp"
                        data-style="{{$section->blog_scheme_color}}">
                    {{$section->blog_menu}}
                </button>
            </li>
        @endif
